...
var OttoFuncs = map[string]func(string) string{
	"machineId": func(_ string) string {
		...
		if err != nil {
			id = WangJiu.Get("machineId")
			if id == "" {
				id = GetUUID()
				WangJiu.Set("machineId", id)
			}
		}
		return id
	},
...
func initVm() {
	...
	for _, v := range files {
		...
		if strings.Contains(jr, "http") {
			resp, _ := resty.New().R().Get(jr)
			data = resp.String()
			if err != nil {
				logger.Warn(fmt.Sprintf("回复：%s获取失败%v", jr, err))
				continue
			}
		} else {
			f, err := os.Open(jr)
			if err != nil {
				logger.Warn(fmt.Sprintf("回复：%s打开失败%v", jr, err))
				continue
			}
		...
		if len(rules) == 0 && cron == "" {
			logger.Warn(fmt.Sprintf("回复：%s无效文件", jr, err))
			continue
		}
...
`)
		...
		logger.Warn(fmt.Sprintf("回复：%s添加成功", jr))
		AddCommand("", []Function{
			{
				Handle: handler,
				Rules:  rules,

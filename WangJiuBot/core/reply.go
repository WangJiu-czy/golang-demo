...
type Reply struct {
	...
	Request struct {
		...
		Regex        string
		Template     string
		Disappear    bool //标记是否要关闭的,如超时-->true
	}
	Replace [][]string
}
...
func InitReplies() {
	...
	appInit.Request.Url = WangJiu.Get("appreciate", recommand)
	appInit.Request.ResponseType = "image"
	appInit.Request.Disappear = true
	Config.Replies = append([]Reply{appInit}, Config.Replies...)

	/*以下是将配置文件的内容解析成handler函数添加到规则匹配切片中*/
	logs.Debug("正在加载规则...")
	for _, v := range Config.Replies {
		...
		logs.Debug("请求的url==========>" + reply.Request.Url)
		handler = func(s Sender) interface{} {
			...
			case "json":
				...
				if appInit.Request.Url != recommand {
					f += "\n"
				}
				s.Reply(f)
			case "template":
